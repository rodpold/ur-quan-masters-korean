...
def render_label(row,raw):
    ...
    if original.mode!='P' or original.info.get('transparency')!=0:raise ValueError('Unexpected shipyard palette format')
    # Keep ANI colormap 69 active: original ink index 227, transparent index 0.
    im=Image.new('P',(w,h),0);im.putpalette(original.getpalette());im.info=dict(original.info)
    total=len(lines)*7+len(lines)-1
    ...
    for text in lines:
        ...
        im.paste(227,((w-mask.width)//2,y+(7-mask.height)//2),mask);y+=8
...
def add_shipyard(entries,rmp,source):
    ...
    for i,line in enumerate(lines):
        ...
        if i in rows:
            ...
            fields[3:]=[str(n) for n in row['hotspot']];lines[i]=' '.join(fields);data=render_label(row,data)
