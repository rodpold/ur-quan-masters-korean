...
def verify(game, ui_font="compact", report_previews=None):
    ...
    with ZipFile(io.BytesIO(data)) as z, ZipFile(Path(game)/patcher.SOURCE) as source:
        ...
        titles_check=verify_intro_titles(z, source)
        (patcher.ROOT/'docs/intro-title-checks.json').write_text(json.dumps(titles_check,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
        ...
        from check_control_panels import verify_control_panels
        control_check = verify_control_panels(z, source)
        (patcher.ROOT/'docs/control-panel-checks.json').write_text(json.dumps(control_check,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
