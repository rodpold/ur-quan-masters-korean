...
def estimate(game, species="spathi"):
    ...
    translations=json.loads((patcher.ROOT/'translations/dialogue'/f'{species}.ko.json').read_text(encoding='utf-8'))
    rows=[]
    ...
    return {'species':species,'mode':'static_estimate_pending_in_game','subtitle_width':width,'slider_y':107,'leading':leading,'segments':len(rows),'max_estimated_bottom':max(x['estimated_bottom'] for x in rows),'near_or_below_slider':[x for x in rows if x['estimated_bottom']>=107], 'limitations':['Assumes one original text line per voice subtitle segment.','Word wrapping and glyph spacing are conservative approximations; compare with engine.','Does not cover no-speech pagination, speaker animation overlap, or player options.']}

if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--game',type=Path,required=True);p.add_argument('--species',choices=['spathi','safeones'],default='spathi');a=p.parse_args()
    print(json.dumps(estimate(a.game,a.species),ensure_ascii=False,indent=2))
