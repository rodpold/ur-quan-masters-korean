...
def audit(game):
 ...
 with ZipFile(Path(game)/patcher.SOURCE) as z:
  from cutscene_text import load_intro,load_ending,compile_script,compile_ending_wrapper
  intro=load_intro()
  compile_script(z.read(intro['source_path']),intro)
  ending=load_ending()
  ...
  credit_spec=load_credits()
  compile_credits(z.read(credit_spec['source_path']),credit_spec)
  credit_translated=sum(bool(r['edits']) for r in credit_spec['records'])
  from ship_text import load_ships, add_ships
  ship_spec=load_ships()
  add_ships(z,ship_spec)
  ship_counts={r['source_path']:len(r['records']) for r in ship_spec['resources']}
  from check_setup import audit_setup
  setup_report=audit_setup(z.read('base/ui/setupmenu.txt'),json.loads((ROOT/'translations/setup.ko.json').read_text(encoding='utf-8')))
  ...
  for n in z.namelist():
   if n.endswith('.txt') and not n.startswith('base/comm/'):
    ...
    if n==credit_spec['source_path']:status='draft_with_preserved_names_urls_padding'
    if n in ship_counts:status='draft_with_explicit_preserved_captain_identifiers'
    if n=='base/ui/setupmenu.txt':status='translated_draft_runtime_pending'
    if n=='base/gamestrings.txt':status='translated_draft_with_explicit_preserved_records'
    if n in ['base/cutscene/intro/intro.txt','base/cutscene/ending/final.txt']:status='subtitle_draft_runtime_pending'
    other.append({'source_path':n,'source_sha256':hashlib.sha256(data).hexdigest(),'records':len(rr),'translated':len(report_trans) if n.startswith('base/lander/') else setup_report['translated_records'] if n=='base/ui/setupmenu.txt' else ui_report['translated_records'] if n=='base/gamestrings.txt' else sum(row.get('decision')!='preserve_alphanumeric_identifier' for r in ship_spec['resources'] if r['source_path']==n for row in r['records']) if n in ship_counts else credit_translated if n==credit_spec['source_path'] else None,'status':status,'linguistic_review':'pending','in_game_review':'pending'})
 return {'goal':'full_game_korean_localization','complete':False,'orz_concepts':orz_report,'dialogue_records':total,'translated_intro_subtitles':len(json.loads((ROOT/'translations/intro.ko.json').read_text(encoding='utf-8'))['records']),'translated_credit_blocks':credit_translated,'preserved_credit_blocks':len(credit_spec['records'])-credit_translated,'translated_ship_labels':125,'translated_captain_names':sum(1 for r in ship_spec['resources'] for row in r['records'] if row.get('decision')=='translated_draft'),'preserved_captain_identifiers':sum(1 for r in ship_spec['resources'] for row in r['records'] if row.get('decision')=='preserve_alphanumeric_identifier'),'pending_captain_names':sum(len(r['pending_captain_ids']) for r in ship_spec['resources']),'setup_records':setup_report['total_records'],'translated_setup_records':setup_report['translated_records'],'pending_setup_records':setup_report['pending_records'],'ui_records':ui_report['total_records'],'translated_ui_records':ui_report['translated_records'],'pending_ui_records':ui_report['pending_records'],'translated_ending_subtitles':len(ending['records']),'translated_report_records':sum(map(len,reports.values())),'report_records':sum(row['records'] for row in other if row['source_path'].startswith('base/lander/')),'translated_dialogue_records':translated_total,'dialogue_coverage_percent':round(translated_total*100/total,2),'locked_terms':len(lock),'persona_groups':len(per['profiles']),'dialogue':rows,'other_text_resources':other,'limitations':['Coverage counts translated records, not meaning/style quality or rendered layout.','Other text includes technical/credit/name fragments; classify before marking preserved/translated.','Numeric check preserves source digit tokens but does not prove dynamic numeral grammar.']}
