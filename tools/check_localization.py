...
def audit(game):
 ...
 with ZipFile(Path(game)/patcher.SOURCE) as z:
  from cutscene_text import load_intro,compile_script
  intro=load_intro()
  ...
  for n in z.namelist():
   if n.endswith('.txt') and not n.startswith('base/comm/'):
    ...
    if n=='base/cutscene/intro/intro.txt':status='subtitle_draft_runtime_pending'
