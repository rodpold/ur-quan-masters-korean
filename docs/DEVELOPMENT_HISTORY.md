...
아래 내용은 시점별 시험 기록입니다. 현재 설치 안내와 동작은 루트 README.md를 기준으로 합니다.
...
첫 화면의 New Game / Load Game / Super Melee! / Setup / Quit는 사용자 선택에 따라 원본 영어 이미지와 디자인을 유지합니다. 설정 내부와 게임 본문은 계속 한국어로 표시합니다.
...
현재 정식 에이전트 의미 검수 기록은 기본 대사 3451/3451개, 음성판 별도 번역 15/15개, 소개·엔딩 자막 74/74개, 탐사 보고 29/29개, 크레딧 55/55개 블록, 설정 121/121개 레코드입니다. 크레딧은 번역 33개와 원문 보존 22개의 결정 검수를 포함합니다. 이 수치는 현재 파일과 일치하는 검수 기록의 범위이며 번역 정확성, 사용자 승인 또는 게임 전체 완료율을 뜻하지 않습니다. UI·이미지 문구와 실제 게임 화면 검증은 별도로 추적합니다.

공통 UI의 현재 파일 기준 의미 검수는 594/594개입니다. 천체·장치·자원·행성 분류와 저장·메뉴·수치·통신 안내를 원문 대조했습니다. 음역은 기존 용어집 기준이며 사용자 승인과 실기 검증은 별도입니다. 문맥별 번역 파일(ui-overrides.ko.json)이 바뀌면 관련 검수 기록도 다시 확인합니다.

함선 문자열의 정식 의미 검수는 528/528개입니다. 종족명·함종명·약칭 125개와 함장 이름 403개의 번역/보존 결정을 대조했습니다. 외계 이름 음역의 최종 승인과 실제 화면·세이브 검증은 별도입니다. 기존 전체 함선 표의 구조·글리프 검사는 의미 검수와 구분합니다.
