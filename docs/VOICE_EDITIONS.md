...
시린 2개 차이: ABOUT_HOMEWORLD는 기본판 12구간 중 베타 코페르니쿠스 I 위치와 마이콘 관련 다섯 구간이 없어 7구간이다. 음성판에 위치 정보를 임의로 보충하지 않는다. LATER는 기본판의 세 차례 시간 경과 대신 한 구간의 몇 시간 후로 바뀐다. 각 판본의 구간과 후행 공백을 보존했다. 실제 녹음 청취·시점 검토는 별도다.

멜노름 HELLO_AND_DOWN_TO_BUSINESS_2는 기본판 17구간의 메타크론 미래 경고가 음성판에서는 첫 인사 1구간만 남는다. 각 판본의 구간 수와 내용을 따로 번역했으며 기본판의 연도·시점 설명을 음성판에 보충하지 않는다. 실제 녹음·표시 시점 검토는 대기 중이다.
