...
기본 UI PNG 420개 모두에 대해 초기 접촉 시트 육안 검토를 마쳤다. `ui-image-review.json`에 이미지별 상태를 기록한다. 이는 번역 완료/실기 통과가 아니며 초기 확인한 121개 중 34개는 한글 패널, 34개는 한글 제목을 생성했으며, 6개는 부분 적용, 47개는 번역 적용 대기다. 일부 작은 버튼 안내는 별도 확대 검토가 필요하다. 게임 그림은 Git에 넣지 않는다.
...
그림 메뉴 playmenu 0–37 및 64의 상단 제목 39개를 8픽셀 영역에 native 7픽셀 잉크로 치환한다. 배경은 원본처럼 투명하고 글자는 원래 회색이다. 아이콘 픽셀·ANI 순서·핫스폿은 보존한다. 31–34의 HUMAN/CYBORG와 36의 장식 이름은 별도 미번역으로 남긴다. playmenu-heading-checks.json은 정적 검사이며 실기 가독성 확인은 대기다.
