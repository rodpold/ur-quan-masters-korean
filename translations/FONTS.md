...
Galmuri7의 7픽셀 잉크와 8×8 PNG 크기, 가로 간격을 유지한다. TinyFont에 한해 투명 여백을 아래 1줄에서 위 1줄로 옮긴다. 상단 지명은 높이 7, 기준선 6, 글리프 핫스폿 7이므로 글리프 이미지의 첫 줄이 화면 밖에 놓인다. 위 여백으로 보정하면 7픽셀 획이 모두 표시 영역 안에 들어간다. 함장/연료/승무원 HUD 이미지도 1픽셀 아래로 맞춘다. 종족·선택지·로그용 다른 폰트는 이 배치 변경의 대상이 아니다. 실제 게임 재확인 전까지 시험 수정이다.
