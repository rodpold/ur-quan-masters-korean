...
함대 대전 작은 버튼 10개는 5/6px 캔버스를 7px로 늘려 Galmuri7 원래 잉크 높이를 수용한다. 이는 글꼴 축소가 아니며 10px 버튼 간격을 검사한다. 별도 테두리를 넣지 않아 7px 텍스트 높이 전체를 사용한다.

### TinyFont 상단 클리핑 보정 (2026-09-26)

Galmuri7의 7픽셀 잉크와 8×8 PNG 크기, 가로 간격을 유지한다. TinyFont에 한해 투명 여백을 아래 1줄에서 위 1줄로 옮긴다. 상단 지명은 높이 7, 기준선 6, 글리프 핫스폿 7이므로 글리프 이미지의 첫 줄이 화면 밖에 놓인다. 위 여백으로 보정하면 7픽셀 획이 모두 표시 영역 안에 들어간다. 함장/연료/승무원 HUD 이미지도 1픽셀 아래로 맞춘다. 종족 자막·플레이어 선택지 폰트는 변경하지 않는다. 대화 로그는 TinyFont를 공유하므로 글자가 1픽셀 내려가며, 8픽셀 줄 간격 안에서 7픽셀 획과 1픽셀 여백을 유지한다. 사용자는 상단 지명·연료 잘림 해결을 확인했다. 보정 이후 긴 로그 페이지 전환은 별도 실기 확인 대상이다.
