...
**v1.0.0 — Steam판 Free Stars: The Ur-Quan Masters 0.8.0의 첫 한국어 번역 배포판**입니다.
...
Releases의 **Assets → `uqm-korean-1.0.0-windows-x64.zip`**을 받으세요.
Windows 10/11 64비트용이며 Python을 따로 설치할 필요가 없습니다.
`Source code`와 `uqm-korean-1.0.0-source.zip`은 개발자용 소스입니다.
...
패치 프로그램의 2번 메뉴로 실행하면 필요한 옵션이 자동으로 적용됩니다.
Steam의 플레이 버튼을 사용하려면 **게임 속성 → 일반 → 실행 옵션**에 다음을 추가하세요.
...
패치 프로그램은 Steam 실행 옵션을 자동으로 바꾸지 않습니다.
...
게임을 종료하고 패치 프로그램의 **4 패치 제거**를 실행합니다.
Steam 실행 옵션에 직접 추가했던 위 옵션도 제거하세요.
