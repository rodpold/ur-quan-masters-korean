...
`install --ui-font compact`로 오른쪽 목록/로그는 7픽셀, 플레이어 선택지와
헤이스/우르콴 자막은 Galmuri9 9픽셀로 구성합니다.
`translations/dialogue/`에 스크린샷의 첫 경고 방송, 헤이스 첫 인사와 선택지 3개만 번역했습니다.
첫 선택지는 함선 이름 삽입 때문에 2개 레코드이며, 총 6개 레코드입니다.
선택 직후의 후속 대사와 다른 종족 대사는 아직 영문입니다.
기존 음성 패키지가 있으면 원래 음성/타임스탬프 경로를 그대로 연결합니다.
